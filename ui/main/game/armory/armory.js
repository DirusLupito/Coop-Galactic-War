...
$(document).ready(function ()
{
    ...
    function ArmoryViewModel() {
        ...
        self.sortCommanders = function ()
        {
            ...
            // }
            // else
            //     self.commanders(available.concat(locked));
        };

        var catalogCommanders = ko.pureComputed(function()
        {
            return PlayFab.catalog().filter(function (x) {
                // It's a commander - and it's represented by PAnet - and it's represented in the PA catalog
        ...
        self.populateInventory = function()
        {
            if (self.signedInToUbernet())
            {
                ...
                self.commanders(_.map(catalogCommanders(), function(commander)
                {
                    ...
                    commander.locked = !(commander.IsFree || commander.IsOwned ) && (commander.NotForSale || !self.allowMicroTransactions());
                    return commander;
                }));
            }
            else
            {
                self.commanders(_.map(CommanderUtility.getKnownCommanders(), function(id)
                {
                    var spec = CommanderUtility.bySpec.getSpec(id);

                    var commander =
                    {
                        ObjectName: spec.catalog_object_name,
                        DisplayName: spec.display_name,
                        KSBacker: spec.kickstarter_backer,
                        IsFree: true,
                        NotForSale: true,
                        locked: false
                    };
        ...
        self.addCart = function ()
        {
            ...
            if (!self.microTransactionsAvailable())
            {
                self.steamOverlayDisabled();
                return;
            }
        ...
        self.setup = function()
        {
            if (!api.content.ownsTitans())
            {
                ...
                // var $video = $('#titan_content video');

                // if ($video.length)
                //     _.delay(function() { $video[0].play()}, 5000);
            }
        }
    }
