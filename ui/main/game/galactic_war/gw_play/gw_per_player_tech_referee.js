define([
    ...
    var apply = function(referee, options) {
        ...
        if (!_.isFunction(inventory.units) || !_.isFunction(inventory.mods) || !_.isFunction(inventory.minions)) {
            console.log('[GW COOP] Per-player tech referee has invalid inventory units, mods, or minions.');
        ...
        for (var clientIndex = 1; clientIndex < connectedClients.length; clientIndex++) {
            var connectedClient = connectedClients[clientIndex];
            var inventoryDataRecord = _.isFunction(game.findCoopPlayerInventoryData) && game.findCoopPlayerInventoryData(connectedClient);

            if (!inventoryDataRecord) {
                console.log('[GW COOP] Missing co-op player inventory data for client ' + JSON.stringify(connectedClient));
                config.per_player_tech_ready = false;
                referee.config(config);
                done.resolve(false);
                return done.promise();
            ...
            if (!_.isString(inventoryDataRecord.commander) || !inventoryDataRecord.inventory) {
                console.log('[GW COOP] Invalid co-op player inventory data for client ' + JSON.stringify(connectedClient));
                config.per_player_tech_ready = false;
                referee.config(config);
                done.resolve(false);
                return done.promise();
            ...
            if (!_.isFunction(loadedInventory.units) || !_.isFunction(loadedInventory.mods) || !_.isFunction(loadedInventory.minions)) {
                console.log('[GW COOP] Invalid co-op player inventory for client ' + JSON.stringify(connectedClient));
                config.per_player_tech_ready = false;
                referee.config(config);
                done.resolve(false);
                return done.promise();
            ...
            playerCommanders[clientIndex] = stripKnownSpecTag(inventoryDataRecord.commander);
        }
