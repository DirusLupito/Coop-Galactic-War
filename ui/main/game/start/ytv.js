...
// joeld: Made changes to compensate for regressions from switching to this branch.
// Needed to switch due to YouTube v2 API being shut down.

(function(win, doc) {
	'use strict';
	var apiKey = 'AIzaSyCqfiowx25WaMYmJhMlikIvLQZ9TOlwv28';
						...
						handle.onreadystatechange = function(){
							if (handle.readyState === 4){
								if (handle.status === 200) {
									...
									// joeld: Hack to show error text for failure.
									settings.element.style.display = "block";

									console.error("Request for " + url + " failed with status " + handle.status + ". Server response: " + handle.responseText);
								}
							}
						};
			...
			prepare = {
				...
				playlists: function(res){
					if(res && res.items){
						...
						// joeld: Hack to show once playlist has been loaded to work around premature visibility of failure text.
						settings.element.style.display = "block";
					}
				},
			...
			action = {
				...
				endpoints: {
					videoClick: function(e){
						// joeld: Replaced logic with client browser launch API call.
						var target = utils.parentUntil(e.target ? e.target : e.srcElement, 'data-ytv');
						if(target){
							if(target.getAttribute('data-ytv')){
								api.youtube.launchPage(target.getAttribute('data-ytv'));
							}
						}
					},
