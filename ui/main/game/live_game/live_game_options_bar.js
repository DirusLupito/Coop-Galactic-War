...
$(document).ready(function () {
    ...
    function OptionsBarViewModel() {
        ...
        self.simSpeedMultiplierChangedTimer = ko.observable(false);

        self.serverRatePercentageString = ko.computed(function() {
            if (self.simSpeedMultiplierChangedTimer()) {
                return self.simSpeedMultiplier() + "x";
            }
            var serverRate = self.serverRate();
            return serverRate ? Math.round( serverRate * 100 ) + '%' : '';
        });

        self.showServerRate = ko.computed(function() {
            var state = self.state && self.state();
            return state && !state.viewReplay && !state.gameOver;
        });

        self.uberbarNotification = ko.observable(false);
    }
    ...
    handlers.control_state = function (payload) {
        ...
        var sim_speed_multiplier = payload.sim_speed_multiplier;

        if (sim_speed_multiplier && sim_speed_multiplier != model.simSpeedMultiplier()) {
            model.simSpeedMultiplier(sim_speed_multiplier);

            var timer = model.simSpeedMultiplierChangedTimer();
            if (timer)
                clearTimeout(timer);
            timer = setTimeout(function() {model.simSpeedMultiplierChangedTimer(false);}, 1500);
            model.simSpeedMultiplierChangedTimer(timer);
        }
    }
    ...
    handlers.uberbar_notification = function(payload) {
        ...
        if (model.uberBar())
            message = false;
       model.uberbarNotification(message);
    };
