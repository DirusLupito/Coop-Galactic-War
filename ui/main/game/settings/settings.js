    ...
    function SettingsViewModel() {
        ...
        self.back = function ()
        {
            ...
            else
            {
                ...
                var lastSceneUrl = self.lastSceneUrl();
                self.lastSceneUrl(window.location.href);
                window.location.href = lastSceneUrl;
            }
        };
        ...
        self.settingDefinitions = ko.observable(api.settings.definitions); // .extend({ rateLimit: { timeout: 500, method: "notifyWhenChangesStop" }});
        ...
        function SettingItemModel(group, key, options, isEnabled)
        {
            ...
            self.value = ko.observable(undefined).extend({ setting: { 'group': group, 'key': key } });
            self.options = self.sourceOptions = options;
            if (options.callback)
            ...
            self.optionsText = ko.observableArray(); // deprecated

            self.loadSelectOptions = function()
            {
                var finalOptions = self.selectOptions ? self.selectOptions : [];
                ...
                try
                {
                    ...
                    if (_.isFunction(options.options))
                        finalOptions = options.options(self.value.peek());
                    else
                    {
                        finalOptions = _.map(options.options, function (option, index)
                        {
                            if (!_.isArray(option))
                                option = { value: option, text: text[index] };
                ...
                try
                {
                    if (options.deferred && !self.loadSelectOptionsDeferred)
                    {
                        self.loadSelectOptionsDeferred = options.deferred(self.value.peek()).then(function(finalOptions)
                        {
                            if (options.empty)
                                finalOptions.unshift({ text: options.empty, value: undefined });

                            self.selectOptions(finalOptions);

                            if (self.sourceOptions.refresh)
                                self.refreshTimeout = setTimeout(self.reloadSelectOptions, Math.max(5000, self.sourceOptions.refresh * 1000));
                        });
                    }
                }
                catch(e)
            ...
            self.reloadSelectOptions = function()
            {
                self.loadSelectOptionsDeferred = false;
                self.selectOptions(self.loadSelectOptions());
            }

            // overloaded options legacy support for mods

            switch(options.type)
            {
                case 'select':
                    self.selectOptions = ko.observableArray(self.loadSelectOptions());
                    self.options = self.selectOptions;
                    break;
                case 'slider':
                    self.sliderOptions = ko.observable(options.options || {});
                    self.options = self.sliderOptions;
                    break;
                default:
                    break;
            }
        ...
        self.conflictKeyboardSettingsItems = ko.computed(function() {
            return _.filter(self.keyboardSettingsItems(), function (item) {
                var options = item.sourceOptions;
        ...
        self.updateKeyConflicts = function () {
            ...
            var allowConflict = function(a, b) {
                if (a === b)
                    return true;
                var allow = a.sourceOptions.allow_conflicts;
                if (allow === true)
                    return true;
                if (!_.isObject(allow))
                    return false;
        ...
        self.processUnitSpecs = function(specs)
        {
            model.unitSpecs(specs);
        };

        // mods add themselves to this array so we need to subscribe to it so that
        // the settings are reloaded.
        self.skyBoxes = ko.observableArray([{text: "Default", value: ''}]);
        self.skyBoxes.subscribe(function(){
            self.settingsItemMap()['graphics.skybox'].reloadSelectOptions();
        });
        ...
        self.setup = function ()
        {
            ...
            if (api.settings.observableMap.ui && api.settings.observableMap.ui.ui_scale) {
                api.settings.observableMap.ui.ui_scale.subscribe(function(ui_scale) {
                    engine.call('ui.setUIScale', ui_scale.toString());
                })
            }
